...
module.exports = async (req, res, next) => {
  ...
  if (!cookies?.jwtAcc) return res.status(401).json({ message: 'unauthorized jwtAcc' })
  if (!cookies?.jwt) return res.status(401).json({ message: 'unauthorized jwt' })
  ...
  try {
    ...
    if (err.message === "jwt expired") {
      console.log("AcessToken has been expired. it is getting refresh. please wait!")
      ...
      try {
        ...
        if (err.message === "jwt expired") {
          ...
          return res.status(403).json({ auth: false , message: "Your session has been expired! please login again." });
        }else{
